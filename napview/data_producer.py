...
class DataProducer:
    ...
    def connect_brainvision_rda(self, max_retries=None, retry_delay=1):
        ...
        while True:
            ...
            try:
                ...
                # Set a 5-second timeout on the socket so we don't block forever
    ...
    def receive_data_chunk(self, requestedSize, max_wait=5.0):
        """
        Returns a bytearray of length 'requestedSize', or raises ConnectionError if
        no data is received within 'max_wait' seconds.
        """
        returnStream = bytearray()
        start_time = time.time()

        while len(returnStream) < requestedSize:
            # If more than max_wait seconds have passed with no new data, treat as connection lost
            if time.time() - start_time >= max_wait:
                raise ConnectionError(f"No data received for {max_wait} seconds, disconnecting.")

            try:
                needed = requestedSize - len(returnStream)
                databytes = self.con.recv(needed)

                # If we get an empty response, treat it as a closed connection
                if not databytes:
                    raise ConnectionError("No data received from amp (empty read).")
    ...
    def unpack_raw_message(self):
        """
        Reads the 24-byte header, unpacks it, then reads the rest of the message.
        Raises ConnectionError on any failures or timeouts.
        """
        try:
    ...
    def setup_openbci(self):
        try:
            ...
            self.logger.info(f"OpenBCI setup complete: Sample rate: {self.sample_rate}, "
                             f"Channels: {self.n_channels}, Channel names: {self.channel_names}")
        except Exception as e:
    ...
    def send_data_loop(self):
        if self.mode == "Simulator":
            ...
            # Poll at twice the sample rate
            sleep_time = chunk_size / (self.sample_rate * 2)
            ...
            while True:
                try:
                    ...
                    time.sleep(sleep_time or 0.0005)
            ...
            while True:
                try:
                    ...
                    # If valid data is received, send it to LSL and update the timestamp
                    if data:
                        self.push_data_to_lsl(data)
                        last_data_time = time.time()
                    else:
                        # If we've had no data for 5 or more seconds, raise a connection error
                        if (time.time() - last_data_time) >= 5:
                            raise ConnectionError(
                                "No data received from Brainvision amp or connection lost for > 5 seconds."
                            )

                    time.sleep(0.0005)

                except Exception as e:
                    self.logger.error(f"Producer: Connection lost or read error: {e}", exc_info=True)
                    while True:
                        try:
                            self.logger.info("Producer: Attempting to reconnect to Brainvision amplifier...")
                            if hasattr(self, 'con'):
                                self.con.close()
                            time.sleep(1)
                            ...
                            self.logger.info("Producer: Reconnection successful.")
                            lastBlock = -1
                            last_data_time = time.time()
                            break

                        except Exception as e2:
                            self.logger.error(f"Producer: Reconnection failed: {e2}", exc_info=True)
                            time.sleep(1)
